...
With the gate green, convert the candidate into the ten DDE records
(same shapes as [`../delivery-fixture/`](../delivery-fixture/), with
`origin: "REAL"`), pin every file in `hashes.json`, then verify:

```bash
node examples/delivery-fixture/verify-fixture.mjs   # run against the converted fixture
```
