...
""
### GATE 3.1 — Deploy EvidenceRegistryV2 (ONE tx; constructor no-args)
[string]$Go=$null
[switch]$FrozenBytesCheck
if(-not $Go){throw "FAIL-CLOSED: must pass -Go GO"}
# == pre-broadcast fail-closed asserts ==
$cid=Hex64 (RpcP "eth_chainId" @()); if($cid -ne $chainId){throw "FAIL cid=$cid"}
try{$pk=GetPkHex}catch{throw}
$init=((& forge inspect contracts/EvidenceRegistryV2.sol:EvidenceRegistryV2 bytecode 2>$null)|Select-Object -Last 1).Trim()
if($init.Length -lt 400){throw "FAIL bytecode recompute"}
"[3.1] ctor=no-args => data=init(0x-prefixed deploy bytecode, NO abi-encoded ctor suffix — evidence: constructor() takes 0 params)"
Broadcast "3.1" "0x0000000000000000000000000000000000000000" $init "EvidenceRegistryV2 DEPLOY (Core Mainnet)" $Go
"[3.1] wait-for-receipt+verify byte-identical runtime"
Start-Sleep -Seconds 6
$code=(RpcP "eth_getCode" @($anchor,"latest"))
"[3.1] anchor=$anchor  runtimeLen=$($code.Length)  unverified-await-3.5"
