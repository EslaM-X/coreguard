...
""
### GATE 3.5 — Independent verification (read-only, zero-gas; NO tx)
$ev=Join-Path (Split-Path -Parent $repo) "evidence"
foreach($g in @("3.1","3.2","3.3")){ $p=Join-Path $ev "gate-$g.json"; if(-not(Test-Path -LiteralPath $p)){throw "FAIL-CLOSED missing evidence $g"} }
# independent recompute: local EvidenceRegistryV2 runtime vs on-chain runtime
$local=((& forge inspect contracts/EvidenceRegistryV2.sol:EvidenceRegistryV2 runtime-code 2>$null)|Select-Object -Last 1).Trim()
$chain=(RpcP "eth_getCode" @($anchor,"latest"))
if($local -ceq $chain){"[3.5] VERIFIED byte-identical (independent recompute): local EvidenceRegistryV2 runtime === on-chain"}else{throw "FAIL-CLOSED mismatch"}
"[3.5] consensus gate: 3/3 gates evidenced, byte-identical recompute, read-only — awaited human audit"
