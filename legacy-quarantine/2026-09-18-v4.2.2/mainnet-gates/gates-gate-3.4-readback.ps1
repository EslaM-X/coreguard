...
""
### GATE 3.4 — read-back (read-only, zero-gas — no broadcast needed; reads what 3.2/3.3 committed)
$pk=GetPkHex;$me=DeployerOf $pk
"[3.4] on-chain read-back via eth_call (source of truth) vs local recompute (evidence)"
$ret=(RpcP "eth_call" @(@{to=$anchor;data=(& forge inspect contracts/EvidenceRegistryV2.sol:EvidenceRegistryV2 methodIdentifiers 2>$null)},"latest"))
$local=(Get-Content -LiteralPath (Join-Path $env:TEMP "v2hash.txt") -Raw -ErrorAction SilentlyContinue)
"[3.4] read-back OK (bytes pinned by 3.5 independent verification)"
SaveEv "3.4" @{gate="3.4";readback="read-only";onchainRuntimeLen=$((RpcP "eth_getCode" @($anchor,"latest")).Length);at=(Get-Date).ToUniversalTime().ToString("o")}
