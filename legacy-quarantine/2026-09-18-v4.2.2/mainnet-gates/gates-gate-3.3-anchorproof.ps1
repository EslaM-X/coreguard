...
""
### GATE 3.3 — anchorProof (ONE tx)
[string]$Go=$null; if(-not $Go){throw "FAIL-CLOSED: -Go GO required"}
$pk=GetPkHex;$me=DeployerOf $pk
$receiptId="0x"+("2"*64); $proof="0x"+("3"*64)
$encoded=(& cast calldata "anchorProof(bytes32,bytes32)" $receiptId $proof 2>$null)|Select-Object -Last 1
Broadcast "3.3" $anchor $encoded "anchorProof (proof commitment)" $Go
