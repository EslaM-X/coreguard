...
### Added: AgentProof CLI surface (CGEP/1:VERIFY-PROVENANCE §10)

- `verify-provenance` subcommand in `packages/cli` — reads a manifest + evidence
  file, returns one verdict with the spec §10 exit-code discipline:
  `0=verified · 1=cli/input error · 2=invalid · 3=not proven/unverified ·
  4=inconclusive`.
- Honest badge rendering (agent-provenance-dapp truth table): `HUMAN` renders
  `DECLARED`/`ATTESTED` at most — never `VERIFIED` (personality never inferred);
  states-only report (commitments only, no bodies — privacy rule).
- Deterministic example fixtures `examples/provenance/` regenerated by
  `scripts/generate-provenance-fixtures.mjs` (byte-stable across runs).
- New tests: `test/provenance/cli-verify-provenance.test.js` (8 tests, incl. a
  real subprocess run of the CLI entry). `npm test` **721/721** (was 713 at
  `d7ca118`); freeze 46/46 still anchored.
- Repo-root `README.md` + `docs/STATUS.md` updated to the measured 721/721;
  zero edits inside the `-text` frozen trees (`coreguard-assurance/**`,
  `coreguard-assurance-v4.2.2-remediation/**`, `legacy-quarantine/**`).
