...
test("delegation: a single signed link passes in isolation", async () => {
  ...
  const r = verifyDelegationLinkSignature(link, CHAIN_ID);
...
test("delegation: tampered authorized-grantee breaks replay (FAIL)", async () => {
  ...
  const r = verifyDelegationLinkSignature(tampered, CHAIN_ID);
...
test("delegation: valid 2-link chain passes with correct grantee", async () => {
  ...
  const r = verifyDelegationChain(links, { chainId: CHAIN_ID, rootAuthority: root });
  assert.equal(r.valid, true);
...
test("delegation: discontinuous chain (break in authority continuity) is FAIL", async () => {
  ...
  const r = verifyDelegationChain([link1, link2], { chainId: CHAIN_ID, rootAuthority: head.address });
  assert.equal(r.valid, false);
...
test("delegation: root authority mismatch vs tx signer is FAIL", async () => {
  ...
  const r = verifyDelegationChain(links, { chainId: CHAIN_ID, rootAuthority: makeAddress(555) });
  assert.equal(r.valid, false);
  assert.match(r.reason, /root/);
});

test("delegation: empty chain is FAIL (fail-closed)", () => {
  const r = verifyDelegationChain([], { chainId: CHAIN_ID, rootAuthority: makeAddress(1) });
  assert.equal(r.valid, false);
});
...
test("delegation: execution BEFORE validAfter is FAIL at that block", async () => {
  ...
  const r = verifyDelegationChain([link], { chainId: CHAIN_ID, rootAuthority: head.address, executionBlock: "500" });
  assert.equal(r.valid, false);
...
test("delegation: execution AFTER expiresAt is FAIL at that block", async () => {
  ...
  const link = await signedLink({ authority: head.address, grantedTo: grantee.address, validAfter: "1000", expiresAt: "5000", priv: head.priv });
  const r = verifyDelegationChain([link], { chainId: CHAIN_ID, rootAuthority: head.address, executionBlock: "6000" });
  assert.equal(r.valid, false);
...
test("delegation: execution inside [validAfter, expiresAt] passes", async () => {
  ...
  const link = await signedLink({ authority: head.address, grantedTo: grantee.address, validAfter: "1000", expiresAt: "5000", priv: head.priv });
  const r = verifyDelegationChain([link], { chainId: CHAIN_ID, rootAuthority: head.address, executionBlock: "3000" });
  assert.equal(r.valid, true);
});
...
test("delegation: in-manifest revokedAt <= executionBlock makes link unusable (§5b)", async () => {
  ...
  const r = verifyDelegationChain([revoked], { chainId: CHAIN_ID, rootAuthority: head.address, executionBlock: "4999" });
  assert.equal(r.valid, false);
...
test("delegation: chainId mismatch changes digest → replay FAIL", async () => {
  ...
  const r = verifyDelegationLinkSignature(link, "1114");
  assert.equal(r.valid, false);
});