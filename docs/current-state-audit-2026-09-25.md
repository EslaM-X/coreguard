...
| Boundary audit | **PASS — 764 files scanned (committed-tree scope), 0 violations** | `npm run boundary:audit` | `current-state.mjs --check` re-measures vs the snapshot |
...
| Git | `main`, 66 commits ahead of `v0.6.0` (the commit introducing this audit moves it to 67) | `git rev-list v0.6.0..HEAD --count` | recorded in the snapshot as metadata |
