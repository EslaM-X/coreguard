...
- `npm test` = 1095 tests, CI-enforced against `docs/state-snapshot.json` on
...
> DISPUTE_PACKAGE_OK).
>
> The whole repository is machine-audited: 1095 tests enforced against a
...
> 1095 tests CI-enforced on Node 18/20/22; the audit refuses any claim of a
...
> (1095 tests, CI-enforced on Node 18/20/22).
>
...
> 1095 tests CI-enforced on Node 18/20/22, boundary audit 0 violations on the
> committed tree.
>
...
> fresh clone: `npm test` = 1095 tests CI-enforced on Node 18/20/22,
> `npm run boundary:audit` = committed-tree scan, 0 violations — the audit
...
> hash-pinned, and machine-audited (1095 tests CI-enforced on Node 18/20/22;
