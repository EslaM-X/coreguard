...
| **v0.5.9 (2026-09-23, this session)** | **People's Court fixture submission posted (owner's words)** — submission package `docs/peoples-court-fixture-submission-2026-09-23.md` pinned to stable commit **ea59a88** with real pinned results (verifier **10/10 PASS**; adversarial **13/13 caught / 0 survived**; compound **5/5 / 0 survived**; fuzz **50 seed 424242 → 0 survived**; decision `EXECUTION_EVIDENCE_ADMISSIBLE — CONFORMITY_UNDECIDED_BY_ENGINE`); the short cover letter (§4) was **POSTED** on thread #21788 **2026-09-23 04:30:52Z UTC**: fixture declared synthetic at the party/dispute/delivery level with a real Core Mainnet execution anchor; two specific questions (mapping-test usefulness; public-thread vs request-demo path); no funding/partnership/sponsorship/introduction asks. **2026-09-23 follow-up: the counterparty (People's Court / Epistemic Labs) ACCEPTED the synthetic fixture for an exploratory public-thread mapping test** and requested the stable anchor; the owner replied (**14:42:43Z UTC**, comment `DC_kwDOMT5cIs4BG1R4`) with `commit ea59a88` + `examples/delivery-fixture/` (byte-identical to `main`), the permitted-use/disclosure/retention limits (records in the fixture), and the re-stated boundary (no adjudication of delivery conformance). All numbers above are CoreGuard-internal verifier outputs and are not a People's Court endorsement. `npm test` **924/924**; thread record updated; awaiting the public mapping result |
...
| #21788 fixture submission (our §4 + stable-anchor reply, owner's words) | **POSTED** 2026-09-23 04:30:52Z UTC (§4) + 14:42:43Z UTC (stabil-anchor reply: `commit ea59a88` + fixture path + use/disclosure/retention limits + boundary). Counterparty **ACCEPTED** the synthetic fixture for the public-thread mapping test the same day; awaiting the mapping result; no funding/partnership asks sent |
...
- Phase transitions — explicit per-phase GO only; first external data that feeds
  ...
  the synthetic fixture for the public-thread mapping test (2026-09-23) and the
  owner delivered the stable anchor (comment `DC_kwDOMT5cIs4BG1R4`,
  `commit ea59a88` + `examples/delivery-fixture/`); no pilot/partnership/funding
  request may be made before the mapping result and the owner's explicit
  approval path. Core email unacknowledged. Pricing stays
