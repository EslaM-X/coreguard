...
| **v0.5.8 (2026-09-23, this session)** | **Pages-build race closed + Windows-native delivery tests + facts record** — `scripts/pages-settle.mjs` Pages-build settle guard (polls `pages/builds/latest`, min-age 15s, max-wait 180s, 60s API-error fallback, exit 0=settled/1=unsettled/2=usage) wired into DDE's trend step (only a settled Pages build may be pushed over) + `test/ci/pages-settle.test.js` 7/7; `test/delivery/convert-zip.test.js` rewritten pure-JS (no system `unzip` — the 2 local Windows failures are gone; CI/Linux parity) with in-process ZIP verification (EOCD scan + central-directory count + local-header walk + CRC-32 recheck + `inflateRawSync`); Pages root 404 fixed via `docs/index.html` landing (Arabic-first, EN mirrored, leads named pages + intake kit); public-thread replies recorded in `docs/elizaos-thread-replies-record-2026-09-23.md` (facts-only; frozen `docs/counterparty-*` untouched; email channel unacknowledged); `npm test` **919/919**; forge **34/34**; corpus+benchmark **73/73**; mutation **1000/1000 refused, 0 escaped**; 10k **FA 0 · FR 0**; pricing remains LOCKED |
...
- Phase transitions — explicit per-phase GO only; first external data that feeds
  ...
  Core email unacknowledged. Pricing stays LOCKED until the owner's decision.
