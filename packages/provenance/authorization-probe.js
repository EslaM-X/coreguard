/**
 * CoreGuard WS-1 — Authority probe surface (EOA / EIP-1271), additive.
 *
 * Mirrors the Firewall's decision-time authority probe semantics
 * (packages/firewall/authority.js) as WS-1's own recomputable surface: the
 * probe is an INPUT, never a caller-supplied boolean (spec §4.2). EOA path
 * recovers the EIP-712 ManifestDeclaration digest; EIP-1271 path reads
 * `isValidSignature` at the labeled authorityAtState via injected
 * `{ ethCall, getCode }`. Magic is CONTRACT_AUTHORIZATION only, never
 * executor proof (T-FW3).
 *
 * Fail-closed: missing adapter/provider/state ⇒ NOT_RUN (never fabricated,
 * no `latest` fallback); revert / wrong magic / empty code ⇒ NOT_PROVEN.
 */
...
/**
 * Evaluate the WS-1 authority probe for a bound manifestId.
 *
 * @param {object} args
 * @param {object} args.signature        signed envelope ({scheme, signer, r, s, v} | {scheme, signer, bytes})
 * @param {object} args.signerBinding    declared authority ({address, kind})
 ...
 * @param {string} [args.from]           optional caller context (EIP-1271 eth_call); never authorizes
...
export async function probeAuthorization({
  ...
  const sig = signature || {};
  const kind = binding.kind === "EIP1271" ? "EIP1271" : "EOA";
  const path = kind === "EIP1271" ? "EIP1271" : "EOA";

  if (!ADDRESS_RE.test(String(binding.address || ""))) {
  ...
  if (path === "EOA") {
    ...
    try {
      ...
      const recovered = evm.recoverSignerAddress(digest, { r, s, v: sig.v });
      if (String(recovered).toLowerCase() === String(binding.address).toLowerCase()) {
        return { status: "OK", label: "RECOVERED_SIGNER", path, atState: authorityAtState || null };
      }
      return { status: "NOT_PROVEN", label: "SIGNER_MISMATCH", reason: `recovered ${recovered} != declared ${binding.address}`, path, atState: authorityAtState || null };
    } catch (e) {
  ...
  let code;
  try {
    code = String(await contractAuth.getCode({ address: binding.address, block: authorityAtState }) || "");
  } catch (e) {
  ...
  let calldata;
  try {
    calldata = evm.isValidSignatureCalldata(digestHex(evm, evm.typedDataDigest, manifestId, chainId), sig.bytes || "0x");
  } catch (e) {
  ...
  let res;
  try {
    res = await contractAuth.ethCall({ to: binding.address, data: calldata, block: authorityAtState, ...(from ? { from } : {}) });
  } catch (e) {
