...
#   - NEVER makes a network call and NEVER starts a subprocess (SELF-002 fix:
#     v2.0.0 spawned `node` twice; since v2.1.0 the engine performs the
#     structural validation IN-PROCESS via a JSON reflection pass and delegates
#     the independent Node validator to the human reviewer command list)
...
# ---- in-process structural validation (v2.1.0, SELF-002 fix) -----------------
# Replaces the two `node` spawns of v2.0.0. Performs the JSON reflection pass
# (shape + summary recomputation + duplicate ids + blocker/status consistency
# + bounded claim + empty evidence) WITHOUT any subprocess. This is an
# additional engine-side reflection, NOT a replacement for the independent
# Node validator: reviewers still run validate-report.mjs themselves (spec
# §5 — the engine may not run its own independent judge).
function Run-StructuralValidator([string]$reportPath, [bool]$expectReject) {
    $out = "structural-validator: "
    if (-not (Test-Path -LiteralPath $reportPath)) {
        Write-Host ($out + "FAIL (report missing: $reportPath)")
        return $false
    }
    try { $r = Get-Content -LiteralPath $reportPath -Raw | ConvertFrom-Json } catch {
        Write-Host ($out + "FAIL (unparseable JSON)")
        return $false
    }
    $errors = New-Object System.Collections.Generic.List[string]
    if (-not $r -or -not $r.findings) { Write-Host ($out + "FAIL (no findings array)"); return $false }
    $recomputed = [ordered]@{
    ...
    foreach ($k in @('pass','fail','blocked','inconclusive','notApplicable','total')) {
        if ([int]$r.summary.$k -ne [int]$recomputed.$k) { $errors.Add("summary.$k=$($r.summary.$k) != recomputed $($recomputed.$k)") }
    }
    $ids = @($r.findings | ForEach-Object { $_.id })
    $dupIds = @($ids | Group-Object | Where-Object { $_.Count -gt 1 })
    if (@($dupIds).Count -gt 0) { $errors.Add("duplicate ids: $($dupIds.Name -join ', ')") }
    $blockers = @($r.findings | Where-Object { $_.severity -eq 'BLOCKER' -and @('FAIL','BLOCKED','INCONCLUSIVE') -contains $_.status })
    if ($blockers.Count -gt 0 -and $r.status -ne 'BLOCKED') { $errors.Add("status $($r.status) with $($blockers.Count) failing blockers") }
    if (-not (Test-BoundedClaim $r.claim)) { $errors.Add("CLAIM-001 bounded-claim check") }
    foreach ($f in @($r.findings)) {
        if ($null -eq $f.evidence -or ($f.evidence.PSObject.Properties | Measure-Object).Count -eq 0) { $errors.Add("M7 empty evidence on $($f.id)") }
    }
    $valid = ($errors.Count -eq 0)
    if ($valid) { Write-Host ($out + "PASS ($($recomputed.total) findings, 0 errors)") }
    else { Write-Host ($out + "FAIL ($($errors.Count) errors: $($errors -join '; '))") }
    if ($expectReject) { return (-not $valid) } else { return $valid }
}
...
function Invoke-Assurance([hashtable]$Cfg) {
    ...
    if ($Cfg.Manifest -and (Test-Path -LiteralPath $Cfg.Manifest)) {
        ...
        $mBase = if ($Cfg.ContainsKey('ManifestBase') -and $Cfg.ManifestBase) { $Cfg.ManifestBase } else { Split-Path -Parent (Split-Path -Parent $Cfg.Manifest) }
        foreach ($row in (Get-Content -LiteralPath $Cfg.Manifest | Where-Object { $_.Trim() -ne "" })) {
            ...
            # v4.2.2 LEG-001 remediation: 'origPath|quarantinePath' means the entry was
            # physically quarantined; its hash must match the pre-recorded value from the
            # QUARANTINE copy (proving byte preservation), never from a live execution root.
            $origRel = ($rel -split '\|', 2)[0]
            $candRel = if ($rel -match '\|') { ($rel -split '\|', 2)[1] } else { $origRel }
            $full = Join-Path $mBase ($candRel -replace '/', '\')
            if (-not (Test-Path -LiteralPath $full)) { $mEntries += [ordered]@{ file = $origRel; status = 'MISSING' }; $mMis++; continue }
            $act = Get-Sha256Hex $full
            if (Test-HashEq $act $exp) { $mEntries += [ordered]@{ file = $origRel; status = 'MATCH'; resolvedFrom = $candRel }; $mMatch++ } else { $mEntries += [ordered]@{ file = $origRel; status = 'MISMATCH'; expected = $exp; actual = $act; resolvedFrom = $candRel }; $mMis++ }
        }
...
'@
        ...
        # ST21 (v2.1.0, SELF-002 fix): the engine itself must contain ZERO
        # exec/net/dynamic sites — a verification tool that promises
        # "NEVER starts a subprocess" must not spawn node/pwsh/anything.
        $selfCheck = Get-Inventory $PSCommandPath $fp
        Assert "ST21-engine-no-subprocess-sites" ($selfCheck.execSites.Count -eq 0 -and $selfCheck.netSites.Count -eq 0 -and $selfCheck.dynamicSites.Count -eq 0)
    } finally {
...
'@
    function FixtureCfg([string]$kitPath, [switch]$NoManifest) {
        ...
        $c = @{
            ...
            Manifest = ''; ExpectedManifestSha256 = ''; ManifestBase = ''; LegacyRoots = @(); SecretScanRoots = @()
    ...
    try {
        ...
        if (Test-Path -LiteralPath $realReport) {
            ...
            $valOk = (Run-StructuralValidator $tmpReport $true)
        }
...
if ($SelfCheckReport) {
    ...
    if (-not (Run-StructuralValidator $reportPath $false)) { $ok = $false }
    ""
...
if ($NegativeSuite) {
    $Real = @{ CastPath = $CastPath; ExpectedCastSha256 = $ExpectedCastSha256; Manifest = $Manifest; ManifestBase = $ManifestBase }
...
if (-not $ManifestBase) { $ManifestBase = Join-Path (Split-Path -Parent $Root) 'coreguard' }
...
<li><b>engine {{TOOLVER}} (4.2.2 remediation)</b>: SELF-002 root fix (zero subprocess sites), in-process structural validator, ST21</li>
...
'@
    ...
    $manifestObj = [ordered]@{ release = 'coreguard-assurance-4.2.3-post-change-validation'; schemaVersion = $script:SchemaVersion; generatedUtc = $dateUtc; toolVersion = $script:ToolVersion; supersedesVersion = $script:SupersedesVersion; ruleVersion = 'cg41-policy-v1'; nonCircular = $true; freezeNote = '4.2.3 = engine 2.1.1: negative-suite premise guard (scenario 9 fails closed when -Manifest absent); post-change validation cycle per owner directive; v4.2.2 frozen evidence preserved untouched'; remediationRefs = 'CHANGELOG-v4.2.2.md, CHANGELOG-v4.2.3.md'; files = $mfEntries }
